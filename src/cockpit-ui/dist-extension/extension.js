...
var TestCodeLensProvider = class {
  ...
  normalizeTestPath(relativePath) {
...
var TestDecorationProvider = class {
  ...
  updateDecorations() {
    ...
    if (this.activeEditor && (document == null ? void 0 : document.fileName.match(/[tT]\.sol$/)) && !(document == null ? void 0 : document.fileName.match(/[tT]\.s.sol$/))) {
      const text = document.getText();
      const decorations = [];
...
var FoundryTaskProvider = class {
  constructor(controller, logger) {
    ...
    this.controller = controller;
    this.taskEndListener = vscode5.tasks.onDidEndTaskProcess((e) => this.handleTaskEnd(e));
  }
  handleTaskEnd(e) {
    for (const [taskId, execution] of this.executions) {
      if (e.execution === execution) {
        ...
        if (definition.command === "fork") {
          vscode5.commands.executeCommand(ForgeCockpitCommand.GetActiveNodesCommand);
  ...
  resolveTask(task) {
    const definition = task.definition;
    return definition.type === "foundry" ? this.createFoundryTask(definition) : void 0;
  }
  ...
  createFoundryTask(definition) {
    ...
    switch (definition.command) {
      case "test": {
        ...
        execution = new vscode5.ShellExecution("forge", testArgs.filter(Boolean), {
  ...
  async terminateTaskByProcess(port) {
    try {
      await vscode5.commands.executeCommand(ForgeCockpitCommand.TerminateAllTasksCommand);
...
var FoundryProjectController = class {
  constructor(logger) {
    ...
    this.taskProviderDisposable = vscode6.tasks.registerTaskProvider("foundry", this.taskProvider);
  }
  async initialize() {
    ...
    try {
      ...
      if (foundryProjects.length > 1) {
        ...
        this.logger.logToOutput(
          `Other projects found at: ${foundryProjects.slice(1).map((p) => p.fsPath).join(", ")}`
        );
      }
  ...
  async findFoundryProjects(workspaceUri) {
    ...
    try {
      ...
      foundryProjects.sort((a, b) => {
        const aDepth = path2.relative(workspaceUri.fsPath, a.fsPath).split(path2.sep).length;
        const bDepth = path2.relative(workspaceUri.fsPath, b.fsPath).split(path2.sep).length;
        return aDepth - bDepth;
      });
    } catch (error) {
      this.logger.logToOutput(`Error searching for Foundry projects: ${error.message}`);
    }
    return foundryProjects;
  }
  async checkNodeModules() {
    try {
      const packageJsonPath = vscode6.Uri.joinPath(this.workspaceRoot, "package.json");
      const nodeModulesPath = vscode6.Uri.joinPath(this.workspaceRoot, "node_modules");
      ...
      if (!nodeModulesExists) {
        const message = "package.json found but node_modules is missing. Please run 'npm install' or 'yarn install' to install dependencies. Incase your smartcontracts use node_modules for remappings";
  ...
  async loadFoundryConfig() {
    ...
    const foundryTomlUri = vscode6.Uri.joinPath(this.workspaceRoot, "foundry.toml");
    const foundryTomlContent = await vscode6.workspace.fs.readFile(foundryTomlUri);
    const parsedContent = new TextDecoder().decode(foundryTomlContent);
    const foundryConfig = toml.parse(parsedContent);
    this.config = {
      verbosity: "-vvvvv",
      viaIR: ((_b = (_a = foundryConfig.profile) == null ? void 0 : _a.default) == null ? void 0 : _b.via_ir) || false,
      testDir: ((_d = (_c = foundryConfig.profile) == null ? void 0 : _c.default) == null ? void 0 : _d.test) || "test",
      srcDir: ((_f = (_e = foundryConfig.profile) == null ? void 0 : _e.default) == null ? void 0 : _f.src) || "src",
      outputDir: ((_h = (_g = foundryConfig.profile) == null ? void 0 : _g.default) == null ? void 0 : _h.out) || "out",
      scriptDir: ((_j = (_i = foundryConfig.profile) == null ? void 0 : _i.default) == null ? void 0 : _j.script) || "script",
    ...
    vscode6.window.showErrorMessage(
      ...
      if (selection === "Install Foundry") {
        ...
        vscode6.env.openExternal(
          vscode6.Uri.parse("https://book.getfoundry.sh/getting-started/installation")
        );
      } else if (selection === "Learn More") {
        this.logger.logToOutput("User clicked 'Learn More' - opening Foundry documentation");
        vscode6.env.openExternal(vscode6.Uri.parse("https://book.getfoundry.sh/"));
      }
    });
  }
  setupWatchers() {
    ...
    this.createFileWatcher(`${(_c = this.config) == null ? void 0 : _c.srcDir}/**/*.sol`, this.handleSourceChange.bind(this));
    this.createFileWatcher(`${(_d = this.config) == null ? void 0 : _d.testDir}/**/*.sol`, this.handleSourceChange.bind(this));
    this.createFileWatcher(
      `${(_e = this.config) == null ? void 0 : _e.scriptDir}/**/*.sol`,
      this.handleSourceChange.bind(this)
    );
    this.createFileWatcher(((_f = this.config) == null ? void 0 : _f.outputDir) || "out", this.handleFileChange.bind(this));
  }
  createFileWatcher(pattern, changeHandler) {
    if (!this.workspaceRoot) {
      return;
    }
    const filePattern = new vscode6.RelativePattern(this.workspaceRoot, pattern);
    const watcher = vscode6.workspace.createFileSystemWatcher(filePattern, false, false, false);
  ...
  handleSourceChange(uri) {
    this.logger.logToOutput(`Source file changed: ${path2.basename(uri.fsPath)}`);
    this.debouncedBuild(1500);
  }
  handleFileChange(uri) {
    this.logger.logToOutput(`Output file changed: ${path2.basename(uri.fsPath)}`);
  ...
  debouncedBuild(delay) {
    ...
    this.debounceTimer = setTimeout(async () => {
      ...
      vscode6.commands.executeCommand(ForgeCockpitCommand.LoadCockPitWalletsCommand);
    }, delay);
  }
  async triggerBuild() {
  ...
  async getExecutablePath(name) {
    ...
    const likelyPath = process.platform === "win32" ? `${homeDir}\\.foundry\\bin\\${name}.exe` : `${homeDir}/.foundry/bin/${name}`;
  ...
  async executeCommand(args = [], cwd = ((_a) => (_a = this.workspaceRoot) == null ? void 0 : _a.fsPath)() || "", streamOutput = true) {
    const env3 = { ...process.env };
    const homeDir = process.env.HOME || process.env.USERPROFILE;
    const foundryBinDir = path2.dirname(
      process.platform === "win32" ? `${homeDir}\\.foundry\\bin\\${args[0]}.exe` : `${homeDir}/.foundry/bin/${args[0]}`
    );
    env3.PATH = `${foundryBinDir}${process.platform === "win32" ? ";" : ":"}${env3.PATH || ""}`;
    return new Promise((resolve, reject) => {
      var _a2, _b;
      if (!streamOutput) {
        cp.exec(args.join(" "), { cwd, env: env3 }, (error, stdout2, stderr2) => {
          if (error) {
      ...
      const child = cp.spawn(command, commandArgs, { cwd, env: env3, shell: true });
      let stdout = "";
      let stderr = "";
  ...
  async executeBuild(useViaIr) {
    ...
    try {
      ...
      const args = [forgePath, "build", "--contracts", `./${(_a = this.config) == null ? void 0 : _a.srcDir}`];
      if (useViaIr) {
        args.push("--via-ir");
      }
      const result = await this.executeCommand(args, ((_b = this.workspaceRoot) == null ? void 0 : _b.fsPath) || "", true);
  ...
  async runAllTests(config2) {
    ...
    try {
      ...
      if (config2.verbosity) {
        args.push(config2.verbosity);
      }
      const workingDirectory = typeof this.workspaceRoot === "string" ? this.workspaceRoot : ((_a = this.workspaceRoot) == null ? void 0 : _a.fsPath) || process.cwd();
      return new Promise((resolve) => {
        var _a2, _b;
        ...
        let output = "";
        let errorOutput = "";
        (_a2 = forgeProcess.stdout) == null ? void 0 : _a2.on("data", (data) => {
          output += data.toString();
        });
        (_b = forgeProcess.stderr) == null ? void 0 : _b.on("data", (data) => {
...
`
  ...
  parseTestResults(output) {
    ...
    try {
      ...
      Object.keys(json).forEach((contractKey) => {
        ...
        const fileName = ((_a = contractKey.split(":")[0]) == null ? void 0 : _a.split("/").pop()) || contractKey;
        Object.keys(testResults).forEach((testKey) => {
          ...
          const error = !isSuccess ? testResult.reason || "Unknown error" : void 0;
          if (testKey.startsWith("Found ") && testKey.includes("instances:")) {
            const testNamesMatch = testKey.match(/instances: (.+)$/);
            const testNames = ((_a2 = testNamesMatch == null ? void 0 : testNamesMatch[1]) == null ? void 0 : _a2.split(", ")) || [];
  ...
  async runScript(contractInfo) {
    ...
    try {
      const account = this.accounts.find(
        (account2) => (0, import_viem2.checksumAddress)(account2.publicKey) === (0, import_viem2.checksumAddress)(contractInfo.msgSender)
      );
      if (!account) {
        return {
          success: false,
      ...
      const result = await this.executeCommand(args, ((_a = this.workspaceRoot) == null ? void 0 : _a.fsPath) || "", true);
      if (result.exitCode !== 0) {
        this.logger.logToOutput(
  ...
  async extractDeployedContracts(contractInfo) {
    ...
    try {
      ...
      const runLatestFiles = await vscode6.workspace.findFiles(
        new vscode6.RelativePattern(broadcastDir, `**/${contractInfo.chainId}/run-latest.json`)
      );
      ...
      this.logger.logToOutput(`Failed to read deployment files: ${error.stack}`);
    }
  ...
  async loadWallets() {
    ...
    try {
      ...
      const cockpitAccounts = vscode6.Uri.joinPath(this.workspaceRoot, "cockpit-accounts.json");
      ...
      if (!exists) {
        ...
        try {
          ...
          this.logger.logToOutput(`Error writing default accounts: ${error.message}`);
        }
      }
  ...
  async getAllContractABIs() {
    ...
    this.logger.logToOutput(`Scanning for contract ABIs in ${(_a = this.config) == null ? void 0 : _a.outputDir}`);
    const outputDirUri = vscode6.Uri.joinPath(this.workspaceRoot, (_b = this.config) == null ? void 0 : _b.outputDir);
    const contracts = [];
    try {
      const jsonPattern = new vscode6.RelativePattern(outputDirUri, "**/*.json");
      ...
      for (const fileUri of jsonFiles) {
        ...
        try {
          ...
          const bytecode = content.bytecode && content.bytecode.object ? content.bytecode.object : "";
  ...
  getScriptName(filePath) {
    if (filePath) {
      const parentDir = path2.dirname(filePath);
      const solFileName = path2.basename(parentDir);
      return solFileName;
    }
    return "";
  }
  async isForgeInstalled() {
    return new Promise((resolve) => {
      let command = process.platform === "win32" ? "where forge 2>nul" : "which forge 2>/dev/null || command -v forge 2>/dev/null";
      cp.exec(command, (error, stdout) => {
        ...
        const homeDir = process.env.HOME || process.env.USERPROFILE;
        const foundryPath = process.platform === "win32" ? `${homeDir}\\.foundry\\bin\\forge.exe` : `${homeDir}/.foundry/bin/forge`;
  ...
  async forkNode(forkInfo) {
    ...
    if (!this.isFoundryProject || !this.workspaceRoot || inUse) {
      const reason = !this.isFoundryProject ? "not a Foundry project" : !this.workspaceRoot ? "no workspace" : "port already in use";
...
var TestParserProvider = class {
  constructor(config2, logger) {
    ...
    __publicField(this, "debounceTimer");
    __publicField(this, "onDidChangeContracts", this._onDidChangeContracts.event);
  ...
  async initialize() {
    ...
    const testDirUri = vscode7.Uri.joinPath(this.config.workspaceRoot, this.config.testDir);
    ...
    for (const fileUri of solFiles) {
      ...
      try {
        ...
        const relativeToWorkspace = path3.relative(this.config.workspaceRoot.fsPath, fileUri.fsPath).replace(/\\/g, "/");
        this.contracts.push({
          fileName,
...
var ForgeTestProvider = class {
  constructor(contracts, onDidChangeContracts, foundryController, logger) {
    ...
    __publicField(this, "currentCancellationTokenSource", new vscode8.CancellationTokenSource());
  ...
  disposeTestItem(item) {
    var _a;
    (_a = item.children) == null ? void 0 : _a.forEach((child) => this.disposeTestItem(child));
  }
  buildTestStructure() {
    const rootItem = this.testController.createTestItem("test", "test", vscode8.Uri.file("test"));
  ...
  addTestContract(contract) {
    ...
    if (((_a = contract.tests) == null ? void 0 : _a.length) > 0) {
      contract.tests.forEach(
        (test) => this.addTestMethod(contractItem, contract.filePath, test, contractUri)
      );
    }
  }
  ...
  async runHandler(request, token, viaIR, verbose) {
    ...
    if (request.include) {
      ...
      this.testController.items.forEach(
        (item) => this.collectTestItems(item, testsToRun, request.exclude)
      );
    }
    ...
    if (this.currentCancellationTokenSource.token.isCancellationRequested || token.isCancellationRequested) {
    ...
    try {
      if (testsToRun.length === 1) {
        ...
        if (!testInfo) {
          run.failed(testsToRun[0], new vscode8.TestMessage("Test info not found"));
        ...
        testsToRun.forEach((test) => {
          ...
          const testFileName = (_a = test.id.split(":")[0]) == null ? void 0 : _a.split("/").pop();
          ...
          if (result && result.success) {
            ...
            run.failed(
              test,
              new vscode8.TestMessage(`Test failed: ${result.error || "Test failed"}`)
            );
            run.appendOutput(
            ...
            run.failed(test, new vscode8.TestMessage(`Test not found in results`));
      ...
      testsToRun.forEach((test) => {
        ...
        const message = new vscode8.TestMessage(`Execution error: ${error.message}`);
  ...
  collectTestItems(item, collected, excluded) {
    ...
    (_a = item.children) == null ? void 0 : _a.forEach((child) => this.collectTestItems(child, collected, excluded));
  }
  runTest(test) {
    const testId = `${test.contractName}:${test.testName}`;
    const testItem = this.testItems.get(testId);
    this.logger.logToOutput(`Running test: ${test.contractName} ${test.testName} found test: ${testItem == null ? void 0 : testItem.label}`);
    if (testItem) {
      const request = new vscode8.TestRunRequest([testItem]);
      this.runHandler(request, new vscode8.CancellationTokenSource().token, false);
    } else {
      this.refreshTests();
      this.logger.logToOutput(`After: Running test: ${test.contractName} ${test.testName} found test: ${testItem == null ? void 0 : testItem.label}`);
    }
  }
  runTestViaIR(test) {
    const testId = `${test.contractName}:${test.testName}`;
    ...
    if (testItem) {
      const request = new vscode8.TestRunRequest([testItem]);
      this.runHandler(request, new vscode8.CancellationTokenSource().token, true);
    }
  }
  runGroup(groupId) {
...
var _ForgeCockPitPanel = class _ForgeCockPitPanel {
  ...
  static render(context) {
    if (_ForgeCockPitPanel.currentPanel) {
      _ForgeCockPitPanel.currentPanel._panel.reveal(import_vscode.ViewColumn.One);
    } else {
      const panel = import_vscode.window.createWebviewPanel("showForgeCockPit", "Forge CockPit", import_vscode.ViewColumn.One, {
  ...
  static isVisible() {
    var _a, _b;
    return (_b = (_a = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _a._panel.active) != null ? _b : false;
  }
  static exists() {
  ...
  _getWebviewContent(webview, context) {
    ...
    const styleUri = webview.asWebviewUri(
      import_vscode.Uri.joinPath(context.extensionUri, "dist", "assets", "index.css")
    );
    const scriptUri = webview.asWebviewUri(
      import_vscode.Uri.joinPath(context.extensionUri, "dist", "assets", "index.js")
    );
    ...
    const html = `
			...
			`;
    import_vscode.commands.executeCommand("forge-cockpit.log", html);
    return html;
  }
      ...
      async (message) => {
        ...
        switch (command) {
          ...
          case WebviewCommand.StopNodeCommand:
            ...
            (_a = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _a._panel.webview.postMessage({
          ...
          case WebviewCommand.DeployContractCommand:
            ...
            const DeployedContract = await import_vscode.commands.executeCommand(
              ForgeCockpitCommand.DeployContractCommand,
              contractInfo
            );
            (_b = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _b._panel.webview.postMessage({
          ...
          case WebviewCommand.WalletBalancesCommand:
            ...
            (_c = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _c._panel.webview.postMessage({
          ...
          case WebviewCommand.ExecuteFunctionCommand:
            ...
            (_d = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _d._panel.webview.postMessage({
          ...
          case WebviewCommand.TransferCommand:
            ...
            (_e = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _e._panel.webview.postMessage({
          ...
          case ClipBoardTypeCommand.ReadClipboard:
            ...
            import_vscode.window.showInformationMessage(`Read from clipboard ${JSON.stringify(text)}`);
            (_f = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _f._panel.webview.postMessage({
          ...
          case WebviewCommand.WriteClipboardCommand:
            ...
            await import_vscode.env.clipboard.writeText(JSON.stringify(payload));
            break;
          case WebviewCommand.RunScriptCommand:
            ...
            (_g = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _g._panel.webview.postMessage({
            ...
            break;
          case WebviewCommand.StopNodeCommand:
            import_vscode.commands.executeCommand(ForgeCockpitCommand.StopNodeCommand, payload);
            break;
          case WebviewCommand.OpenLinkCommand:
            import_vscode.commands.executeCommand(ForgeCockpitCommand.OpenUrlCommand, payload);
            break;
          case WebviewCommand.AbiEncodeCommand:
            const input = payload;
            const encoded = encodeFunction(input);
            (_h = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _h._panel.webview.postMessage({
          ...
          case WebviewCommand.LoadContractsCommand:
            import_vscode.commands.executeCommand(ForgeCockpitCommand.RefreshTestsCommand);
            import_vscode.commands.executeCommand(ForgeCockpitCommand.LoadCockPitWalletsCommand);
            return;
          case WebviewCommand.ForkNodeCommand:
            ...
            (_i = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _i._panel.webview.postMessage({
  ...
  static async sendActiveNodes(command) {
    var _a;
    if (_ForgeCockPitPanel.currentPanel) {
      const nodes = await import_vscode.commands.executeCommand(ForgeCockpitCommand.GetActiveNodesCommand);
      (_a = _ForgeCockPitPanel.currentPanel) == null ? void 0 : _a._panel.webview.postMessage({
...
var AbiProvider = class {
  ...
  setupFileWatcher() {
    ...
    try {
      const pattern = new vscode11.RelativePattern(workspaceFolders[0], `${outputDir}/**/*.json`);
...
var StubTestProvider = class {
  constructor(contractName, abi, logger) {
    ...
    __publicField(this, "patterns", [
      {
        name: "foundry-uups",
        detect: (code, abi) => code.includes("openzeppelin-foundry-upgrades") && (code.includes("UUPS") || this.hasInitializer(abi)),
        template: {
      ...
      },
      {
        ...
        detect: (code, abi) => code.includes("openzeppelin-foundry-upgrades") && code.includes("Transparent"),
        template: {
          imports: 'import {UnsafeUpgrades} from "openzeppelin-foundry-upgrades/Upgrades.sol";',
          declarations: "address public proxy;",
      ...
      {
        name: "erc1967-proxy",
        detect: (code, abi) => (code.includes("upgradeable") || code.includes("proxy") || this.hasInitializer(abi)) && !code.includes("foundry-upgrades"),
        template: {
    ...
    __publicField(this, "baseTemplate", `// SPDX-License-Identifier: UNLICENSED
...
`);
  ...
  async generateTestFile(filePath) {
    var _a;
    try {
      const workspaceFolder = (_a = vscode12.workspace.workspaceFolders) == null ? void 0 : _a[0];
      if (!workspaceFolder) {
        throw new Error("No workspace folder found.");
      }
      const wsName = import_path.default.basename(workspaceFolder.uri.fsPath);
      let normalized = filePath;
      if (normalized.startsWith(wsName + import_path.default.sep)) {
        normalized = normalized.slice(wsName.length + 1);
      }
      const absolutePath = import_path.default.join(workspaceFolder.uri.fsPath, normalized);
      const sourceCode = await vscode12.workspace.fs.readFile(vscode12.Uri.file(absolutePath));
  ...
  extractStateVariables() {
    ...
    const stateVarRegex = /(?:uint256|uint\d*|int256|int\d*|bool|address|string|bytes\d*|mapping\([^)]+\)|struct\s+\w+|\w+)\s+(public|private|internal|constant|immutable)\s+(\w+)(?:\s*=\s*[^;]+)?;/g;
    let match;
    ...
    while ((match = emitRegex.exec(functionBody)) !== null) {
      ...
      if (abiEvent) {
        const eventInfo = {
          ...
          indexed: ((_a = abiEvent.inputs) == null ? void 0 : _a.filter((input) => input.indexed).map((input) => input.name)) || [],
          nonIndexed: ((_b = abiEvent.inputs) == null ? void 0 : _b.filter((input) => !input.indexed).map((input) => input.name)) || [],
    ...
    if (this.sourceCode && functionName) {
      ...
      if (functionBodyMatch && functionBodyMatch[0]) {
        ...
        for (const [varName, varInfo] of this.stateVariables) {
          if (functionBody.includes(`${varName} =`) || functionBody.includes(`${varName}++`) || functionBody.includes(`${varName}--`) || functionBody.includes(`++${varName}`) || functionBody.includes(`--${varName}`) || functionBody.includes(`${varName}[`) || functionBody.includes(`${varName}.`)) {
            modifiesState = true;
        ...
        canRevert = functionBody.includes("require(") || functionBody.includes("revert(") || functionBody.includes("assert(") || functionBody.includes("_revert");
      }
    }
    if (!modifiesState) {
...
${pattern.template.imports}` : "",
  ...
  renderDependencyImports() {
    const imports = Array.from(this.dependencies).map((dep) => `import "src/${dep}.sol";`).join("\n");
    return imports ? `
${imports}` : "";
  }
  renderDependencyDeclarations() {
    ...
    const declarations = Array.from(this.dependencies).map((dep) => `${dep} public ${dep.toLowerCase()};`).join("\n    ");
    return `
    ${declarations}`;
  }
  renderDependencySetup() {
    ...
    const setup = Array.from(this.dependencies).map((dep) => `        ${dep.toLowerCase()} = new ${dep}();`).join("\n");
    return `${setup}
`;
  }
  getInitData() {
    ...
    return ((_a = initializer == null ? void 0 : initializer.inputs) == null ? void 0 : _a.length) ? `abi.encodeCall(${this.contractName}.initialize, (${this.buildParamList(initializer.inputs)}))` : '""';
  }
  getConstructorParams() {
    const constructor = this.abi.find((item) => item.type === "constructor");
    return (constructor == null ? void 0 : constructor.inputs) ? this.buildParamList(constructor.inputs) : "";
  }
  ...
  getDefaultValue(input) {
    ...
    if ((type == null ? void 0 : type.startsWith("bytes")) && type !== "bytes") {
      return '"0x01"';
    }
  ...
  buildStructLiteral(input) {
    ...
    const fields = input.components.map((comp) => `${comp.name}: ${this.getDefaultValue(comp)}`).join(", ");
    ...
    if (this.sourceCode && input.name) {
      ...
      if (structMatches) {
        for (const structMatch of structMatches) {
          const structName = (_b = structMatch.match(/struct\s+([A-Za-z_][A-Za-z0-9_]*)/)) == null ? void 0 : _b[1];
          if (structName && structName.toLowerCase() === input.name.toLowerCase()) {
            return structName;
          }
        }
        if (structMatches.length > 0) {
          const firstStruct = (_c = structMatches[0].match(/struct\s+([A-Za-z_][A-Za-z0-9_]*)/)) == null ? void 0 : _c[1];
          if (firstStruct) {
            return firstStruct;
          }
        }
  ...
  renderTests() {
    const functions = this.abi.filter(
      (item) => item.type === "function" && item.name && item.name !== "initialize" && !this.isStateVariableGetter(item)
    );
    let allTests = "";
  ...
  isStateVariableGetter(func) {
    var _a, _b;
    if (!this.sourceCode || func.stateMutability !== "view" || ((_a = func.inputs) == null ? void 0 : _a.length) !== 0) {
      return false;
    }
    return this.stateVariables.has(func.name) && ((_b = this.stateVariables.get(func.name)) == null ? void 0 : _b.visibility) === "public";
  }
...
`;
  ...
  generateEventAssertions(functionName) {
    ...
    const assertions = functionEvents.map((event, index) => {
  ...
  generateViewAssertion(output, varName, analysis) {
    ...
    if ((type == null ? void 0 : type.startsWith("uint")) || (type == null ? void 0 : type.startsWith("int"))) {
    ...
    if (this.isStructType(output)) {
      ...
      if (output.components && output.components.length > 0) {
        const fieldChecks = output.components.map((comp) => `assertTrue(true);`).join("\n        ");
        return fieldChecks;
      }
  ...
  renderRevertTests(func) {
    ...
    if (modifiers.length > 0) {
      revertTests += modifiers.map((modifier) => {
    ...
    if (func.inputs && func.inputs.length > 0) {
      const zeroParams = func.inputs.map((input) => {
        var _a, _b;
        if (input.type === "address") {
          return "address(0)";
        }
        if (((_a = input.type) == null ? void 0 : _a.startsWith("uint")) || ((_b = input.type) == null ? void 0 : _b.startsWith("int"))) {
          return "0";
        }
  ...
  extractFunctionModifiers(functionName) {
    ...
    const potentialModifiers = words.map((word) => word.replace(/\([^)]*\)$/, "")).filter((word) => !solidityKeywords.has(word) && word.length > 0).filter((word) => this.isModifierDefined(word));
  ...
  renderFuzzTests() {
    const functions = this.abi.filter(
      (item) => {
        var _a;
        return item.type === "function" && ((_a = item.inputs) == null ? void 0 : _a.length) && item.name && item.name !== "initialize" && !this.isStateVariableGetter(item);
      }
    );
  ...
  renderFuzzTest(func) {
    const name = this.toPascalCase(func.name);
    const params = func.inputs.map((input, i) => {
      const type = this.getType(input);
      const paramName = input.name || `param${i}`;
      return `${type} ${paramName}`;
    }).join(", ");
    const callParams = func.inputs.map((input, i) => input.name || `param${i}`).join(", ");
    const analysis = this.analyzeFunction(func);
  ...
  async getBalances(wallets) {
    try {
      const balances = await Promise.all(
        wallets.map(
          (address) => this.publicClient.getBalance({
            address
          })
        )
      );
      const formattedBalances = balances.map((balance) => (0, import_viem3.formatEther)(balance).toString());
      this.logger.logToOutput(
...
var CockPitLogProvider = class {
  constructor() {
    ...
    this.outputChannel = vscode13.window.createOutputChannel("Forge Cockpit Logs");
    this.buildStatusBar = vscode13.window.createStatusBarItem(vscode13.StatusBarAlignment.Left, 100);
  ...
  updateStatusBar(message, theme) {
    ...
    this.buildStatusBar.backgroundColor = theme != null ? theme : new vscode13.ThemeColor("statusBarItem.successBackground");
  }
};
...
async function activate(context) {
  ...
  if (foundryProjectController.isFoundry()) {
    ...
    const actionsTreeView = vscode14.window.createTreeView("forgeCockpitActions", {
    ...
    context.subscriptions.push(
      vscode14.commands.registerCommand(
        ...
        async (contract) => {
          ...
          const abi = (_a = abiProvider.abis.find((a) => a.fileName === contract.fileName)) == null ? void 0 : _a.abi;
          if (!abi) {
            vscode14.window.showErrorMessage(`ABI not found for ${abiProvider.abis.length} ABIs`);
            return;
          }
      ...
      vscode14.commands.registerCommand(ForgeCockpitCommand.ShowForgeCockPitCommand, () => {
        initWebView(context);
        vscode14.commands.executeCommand(ForgeCockpitCommand.PinEditorCommand);
        vscode14.commands.executeCommand(ForgeCockpitCommand.LoadCockPitWalletsCommand);
      }),
      vscode14.commands.registerCommand(ForgeCockpitCommand.RebuildProjectCommand, () => {
      ...
      vscode14.commands.registerCommand(ForgeCockpitCommand.RunGroupCommand, (path6) => {
      ...
      vscode14.commands.registerCommand(
        ...
        async (callInfo) => {
          ...
          const abi = (_a = abiProvider.abis.find((a) => a.fileName === callInfo.contractAddress)) == null ? void 0 : _a.abi;
      ...
      vscode14.commands.registerCommand(
        ...
        async (port) => {
          ...
          if (success && cachedNodes) {
            const newNodes = cachedNodes == null ? void 0 : cachedNodes.filter((url) => !url.includes(port));
      ...
      vscode14.commands.registerCommand(
        ...
        async () => {
          ...
          const allNodes = [.../* @__PURE__ */ new Set([...nodes, ...anvilNodes])];
          logger.logToOutput(`Found ${allNodes} anvil cached nodes`);
          return allNodes;
        }
      ),
      vscode14.commands.registerCommand(ForgeCockpitCommand.ShowBuildOutputCommand, () => {
        logger.showBuildOutput();
      }),
      vscode14.commands.registerCommand(ForgeCockpitCommand.OpenUrlCommand, (url) => {
        vscode14.commands.executeCommand(ForgeCockpitCommand.VsOpenUrlCommand, vscode14.Uri.parse(url));
      }),
      vscode14.commands.registerCommand(
          ...
          if (cachedNodes) {
            await Promise.all([
              cachedNodes.map((nodeUrl) => {
                ...
                return vscode14.commands.executeCommand(ForgeCockpitCommand.StopNodeCommand, port);
              })
            ]);
          }
