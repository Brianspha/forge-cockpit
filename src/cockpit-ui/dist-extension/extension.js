...
`
  ...
  async runScript(contractInfo) {
    ...
    this.logger.logToOutput(
      `Starting script execution: ${contractInfo.contractName} in ${contractInfo.scriptName} and details ${JSON.stringify(contractInfo)}`
    );
    this.logger.updateStatusBar(
...
`);
  ...
  async generateTestFile(filePath) {
    ...
    try {
      ...
      const absolutePath = import_path.default.join(workspaceFolder.uri.fsPath, filePath);
    ...
    context.subscriptions.push(
      ...
      vscode14.commands.registerCommand(
        ...
        async (config2) => {
          ...
          logger.logToOutput(`contract: ${JSON.stringify(contract)}`);
          if (!contract) {
            return {
              success: false,
              contracts: [
